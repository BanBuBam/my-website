...
import { nurseEmergencyAPI, nurseEncounterListAPI } from '../../../../services/staff/nurseAPI';
import {
...
const EmergencyListPage = () => {
    ...
    const fetchWaitingTriageEncounters = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await nurseEmergencyAPI.getWaitingTriageEncounters();
    ...
    const fetchEmergencies = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await nurseEmergencyAPI.getActiveEmergencies();
            if (response && response.data) {
    ...
    const fetchEncounters = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await nurseEncounterListAPI.getEncounters(currentPage, pageSize, ['startDatetime,desc']);
            if (response && response.data) {
    ...
    const handleViewEmergencyDetail = (emergencyEncounterId) => {
        navigate(`/staff/dieu-duong/cap-cuu/emergency/${emergencyEncounterId}`);
    };

    const handleViewEncounterDetail = (encounterId) => {
        navigate(`/staff/dieu-duong/cap-cuu/encounter/${encounterId}`);
    };

    const handleRefresh = () => {
        if (activeTab === 'waiting-triage') fetchWaitingTriageEncounters();
        else if (activeTab === 'emergencies') fetchEmergencies();
        else if (activeTab === 'encounters') fetchEncounters();
        else if (activeTab === 'create') fetchCreateEmergencies();
    };

    const handleCreateNew = () => {
        navigate('/staff/dieu-duong/cap-cuu/tao-moi');
    };
