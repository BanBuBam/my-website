...
import { FiPlus, FiLock, FiKey, FiSearch, FiEye, FiEdit2, FiTrash2, FiFilter, FiUserCheck, FiUserX } from 'react-icons/fi';
...
const AccountManagementPage = () => {
  ...
  const handleAddAccount = async (accountData, mode) => {
    try {
      console.log('📝 Creating account with mode:', mode);
      ...
      if (mode === 'existing') {
        ...
        response = await hrAccountAPI.createAccountForExistingEmployee(accountData);
        ...
        console.log('🔄 Calling createEmployeeWithAccount API...');
        response = await hrAccountAPI.createEmployeeWithAccount(accountData);
        console.log('✅ Create employee with account response:', response);

        if (response && response.status === 'OK') {
          alert('Tạo nhân viên và tài khoản thành công!');
        } else {
          throw new Error(response?.message || 'Không thể tạo nhân viên và tài khoản');
        }
      }
      ...
      <div className="filter-section">
        ...
        <div className="filter-controls">
          <div className="search-box">
            ...
            <input
              type="text"
              placeholder="Tìm kiếm theo tên, username, mã nhân viên..."
              value={searchTerm}
