...
import './AddTimeOffRequestModal.css';

const AddTimeOffRequestModal = ({ isOpen, onClose, onSuccess, employeeId }) => {
  ...
  const handleInputChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // Tính toán lại totalDays khi thay đổi ngày
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await hrTimeOffAPI.createTimeOffRequest(payload);
      ...
      if (response.success) {
        ...
        setError(response.error || 'Lỗi khi tạo đơn nghỉ phép');
      }
    } catch (err) {
      setError(err.message || 'Lỗi khi tạo đơn nghỉ phép');
    } finally {
      setLoading(false);
    }
  };
...
export default AddTimeOffRequestModal;
