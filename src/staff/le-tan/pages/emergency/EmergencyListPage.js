...
import { receptionistEmergencyAPI, receptionistEncounterAPI } from '../../../../services/staff/receptionistAPI';
import {
    FiAlertCircle, FiRefreshCw, FiClock, FiUser, FiActivity,
    FiPhone, FiMapPin, FiEye, FiAlertTriangle, FiSearch, FiPlus
} from 'react-icons/fi';
import CreateEmergencyPage from './CreateEmergencyPage';
import './EmergencyListPage.css';

const EmergencyListPage = () => {
    const navigate = useNavigate();
    const [activeTab, setActiveTab] = useState('emergencies'); // 'emergencies', 'encounters', 'create'
    const [emergencies, setEmergencies] = useState([]);
    const [createEmergencies, setCreateEmergencies] = useState([]); // For create tab
    ...
    useEffect(() => {
        if (activeTab === 'emergencies') {
            ...
            // Auto refresh every 30 seconds for create tab
    ...
    const fetchEmergencies = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await receptionistEmergencyAPI.getActiveEmergencies();
    ...
    const fetchEncounters = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await receptionistEncounterAPI.getEncounters(currentPage, pageSize, ['startDatetime,desc']);
    ...
    const handleViewEmergencyDetail = (emergencyEncounterId) => {
        navigate(`/staff/le-tan/cap-cuu/emergency/${emergencyEncounterId}`);
    };

    const handleViewEncounterDetail = (encounterId) => {
        navigate(`/staff/le-tan/cap-cuu/encounter/${encounterId}`);
    };
    ...
    const handleCreateNew = () => {
        navigate('/staff/le-tan/cap-cuu/tao-moi');
    };
