...
import { receptionistEmergencyAPI, receptionistPatientSearchAPI, receptionistDepartmentAPI, receptionistEmployeeAPI } from '../../../../services/staff/receptionistAPI';
...
const CreateEmergencyPage = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                // Load departments
                const deptResponse = await receptionistDepartmentAPI.getDepartments();
                if (deptResponse && deptResponse.data.content) {
                    setDepartments(deptResponse.data.content);
                }

                // Load nurses
                const nurseResponse = await receptionistEmployeeAPI.getNurses(0, 100);
    ...
    const loadDoctorsByDepartment = async (departmentId) => {
        try {
            const response = await receptionistEmployeeAPI.getDoctorsByDepartment(departmentId);
    ...
    const handleSearchPatients = async () => {
        ...
        try {
            ...
            const response = await receptionistPatientSearchAPI.searchPatientsByName(patientSearchQuery, 0, 10);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await receptionistEmergencyAPI.createEmergency(submitData);
